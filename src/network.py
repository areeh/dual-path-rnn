...
class DprnnBlock(keras.layers.Layer):
    ...
    def call(self, T):
        ...
        T_shaped = tf.reshape(
            T,
            (
                self.batch_size * self.num_overlapping_chunks,
                self.chunk_size,
        ...
        U = tf.reshape(
            U,
            (self.batch_size * self.num_overlapping_chunks * self.chunk_size, self.units_per_lstm),
        )
        ...
        U_Hat = tf.reshape(
            ...
            (
                self.batch_size,
                self.num_overlapping_chunks * self.chunk_size * self.num_filters_in_encoder,
            ),
        )
        ...
        V = tf.reshape(
            V,
            (self.batch_size * self.chunk_size * self.num_overlapping_chunks, self.units_per_lstm),
        )
        V_Hat = self.inter_fc(V)
        V_Hat = tf.reshape(
            V_Hat, (self.batch_size, self.num_overlapping_chunks * self.fc_units * self.chunk_size)
        )
        LN_V_Hat = self.inter_ln(V_Hat)
        T_Out = tf.reshape(
            LN_V_Hat, (self.batch_size, self.chunk_size, self.num_overlapping_chunks, self.fc_units)
        )
...
class TasnetWithDprnn:
    ...
    def segment_encoded_signal(self, x):
        x1 = tf.reshape(
            ...
            (
                self.batch_size,
                self.signal_length_samples // self.chunk_size,
                self.chunk_size,
                self.num_filters_in_encoder,
            ),
        )
    ...
    def generate_model(self):
        ...
        # Decode speaker0
        decoded_spk0 = keras.layers.Dense(units=self.encoder_filter_length, use_bias=False)(
            encoded_spk0
        )
        decoded_spk0 = keras.layers.Lambda(self.overlap_and_add_in_decoder)(decoded_spk0)[
            :, : self.signal_length_samples
        ]
        # Decode speaker1
        decoded_spk1 = keras.layers.Dense(units=self.encoder_filter_length, use_bias=False)(
            encoded_spk1
        )
        decoded_spk1 = keras.layers.Lambda(self.overlap_and_add_in_decoder)(decoded_spk1)[
            :, : self.signal_length_samples
        ]
