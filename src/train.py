    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        with valid_summary_writer.as_default():
            tf.summary.audio(
                f"{epoch}_audio",
                np.reshape(
                    output, (self.n_utterances * NUM_SPEAKERS, self.utterance_length_in_samples, 1)
                ).astype(np.float32),
                self.samplerate_hz,
                encoding="wav",
                step=self._total_batches_seen,
                max_outputs=NUM_LOG_AUDIO_FILES_PER_EPOCH,
            )
...
def train_network(
    ...
    # tasnet_data_train = TasnetData(
    #     data_root_dir=wav_data_dir_train,
    #     file_list_path=file_list_path_train,
    #     samplerate_hz=samplerate_hz,
    #     utterance_length_in_seconds=utterance_length_in_seconds,
    #     num_speakers=NUM_SPEAKERS,
    # )
    # tasnet_data_valid = TasnetData(
    #     data_root_dir=wav_data_dir_valid,
    ...
    # store_audio_callback = WriteValidationAudio(
    #     log_dir=os.path.join(tensorboard_dir),
    ...
    plot_model(model=tasnet.model, to_file="arch.png")
